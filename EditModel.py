...
class EditModel(object):
    ...
    def insertEdits(self, word):
        ...
        for i in xrange(1, len(word) + 1):  # insert a letter
            corruptLetters = word[i - 1]
            for j in xrange(0, len(EditModel.ALPHABET)):
                correction = word[1:i] + EditModel.ALPHABET[j] + word[i:]
                correctLetters = str(word[i - 1]) + str(EditModel.ALPHABET[j])
    ...
    def transposeEdits(self, word):
        ...
        for i in xrange(1, len(word)-1):
            ...
            currChar = word[i]
            adjacentChar = word[i+1]

            corruptLetters = currChar + adjacentChar
            correctLetters = adjacentChar + currChar
            correction = word[1:i] + adjacentChar + currChar + word[i+2:]
    ...
    def replaceEdits(self, word):
        ...
        for i in xrange(1, len(word)):
            ...
            for j in xrange(0, len(EditModel.ALPHABET)):
                correction = word[1:i] + EditModel.ALPHABET[j] + word[i+1:]
                #print correction

                correctLetters = EditModel.ALPHABET[j]

                if correction != word[1:]: # don't add duplicate of original, fixme seems to have duplicates
                    ret.append(Edit(correction, corruptLetters, correctLetters))
