...
class SpellCorrect:
    ...
    def correctSentence(self, sentence):
        ...
        for i in xrange(1, len(sentence) - 1):  # ignore <s> and </s>
            # select the maximum probability sentence here, according to the noisy channel model.
            # Tip: self.editModel.editProbabilities(word) gives edits and log-probabilities
            # according to your edit model.
            ...
            for x in self.editModel.editProbabilities(sentence[i]):  # look at sentence[i]
                editProb = x[1]

                temp = sentence[:]
                temp[i] = x[0]  # place edited word into sentence

                logProb = self.languageModel.score(temp)

                totalScore = editProb + logProb
