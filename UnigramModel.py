...
class UnigramModel:
  ...
  def train(self, corpus):
    ...
    # looks at each word in each sentence in corpus, keeps count of number of occurrences
    for sentence in corpus.corpus:
      for datum in sentence.data:
        token = datum.word
        self.unigramCounts[token] = self.unigramCounts[token] + 1 # increment number of times it is seen in the corpus
        self.total += 1
